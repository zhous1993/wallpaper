...
function createWindow() {
    ...
    //     mainWindow.hide()
    // })
    mainWindow.setIcon(path.resolve(__static, './img/trayTemplate.png'))
    ...
    // mainWindow.setThumbnailClip(obj)
    mainWindowShow();
}
    ...
    appTray.on('click', (event, bounds, position) => {
        ...
        function setMainWinPosition(win, trayBounds) {
            try {
                ...
                log.info(currentScreen.bounds);
                const winPositionX = (currentScreen.bounds.width - winWidth) / 2;
                const winPositionY = (currentScreen.bounds.height - winHeight) / 2 - 50;
...
function ipcMainInit() {
    ...
    ipcMain.on('downloadImg', (event, arg) => {
        downloadPic(arg.downloadUrl, mainWindow).then((result) => {
            log.info('下载成功');
            event.sender.send('downloadImg', 'success');
        }).catch((error) => {
            log.error('下载失败');
            event.sender.send('downloadImg', 'error')
        })
    })
    ...
    ipcMain.on('getSign', (event, data) => {
        getOne().then((res) => {
            mainWindow.webContents.send('sign', {
                type: 'success',
                data: res
            })
        })
...
function getOne() {
    return new Promise((resolve, reject) => {
        request('http://wufazhuce.com/', (error, response, body) => {
            const $ = cheerio.load(body)
            const data = $('#carousel-one div.active .fp-one-cita a');
            let title
            data.map((item) => {
                title = data[item].children[0].data;
                return title
            })
            resolve(title)
        })
